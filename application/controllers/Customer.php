...
class Customer extends CI_Controller {
	...
	{  
	    ...
	    $data['sal'] =  $this->setting_model->commonGet($option3);
		$data['keyw'] = $keyword;
		$data['is_arc'] = $chk;
		$data['page'] = 55;
	...
	public function list($keyword = '',$is_archive = '')
	{
		...
		$option1 = array(
			'select' => array('*'),
	    	'table' => 'tperson',
	    	'where' => array('tperson.person_type' => 1),
	    	'where_in' => array(
		...
		$this->load->view('admin/demo/ecommerce_customer',$data);
	...
	public function action_customer()
	{
		...
		$is_except_term=$this->input->post('is_except_term');
		$jenis_harga=$this->input->post('jenis_harga');
		$is_person_limit=$this->input->post('is_person_limit');
		$person_limit_piutang=$this->input->post('person_limit_piutang');
		...
		if ($person_code == '') {
			$person_code = $this->GetNoIDMasterBulan('CS','person_code','tperson');
		}
		...
		if( is_array($data['get_data']) || is_object($data['get_data']) ) {
		    foreach($data['get_data'] as $right) {
		    	echo json_encode(array(
					...
					'npwp' => $right->npwp,
					'is_except_term' => $right->is_except_term,
					'jenis_harga' => $right->jenis_harga,
					'is_person_limit' => $right->is_person_limit,
					'person_limit_piutang' => $right->person_limit_piutang,
					'salesman' => $right->salesman,
	...
	public function print_pdf($keyword = '',$is_archive = '')
	{
		...
		$pdf->Cell($tableX);
		$pdf->Cell($tableWidth, 10, 'DAFTAR Customer OHH_BAKERY', 0, 1, 'C');
		$pdf->Cell(10, 7, '', 0, 1);
	...
	{
		// file name 
		$filename = 'customer_'.date('d-m-Y').'.csv'; 
