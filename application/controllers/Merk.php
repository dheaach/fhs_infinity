...
class Merk extends CI_Controller {
	...
	{  
		...
		$ct1 = array(
			'select' => array('count(tm_merk.merk_id) as aktif'),
	    	'table' => 'tm_merk',
	    	'where' => array(
							'tm_merk.is_delete' => 0)
	    );
	    $ct2 = array(
			'select' => array('count(tm_merk.merk_id) as non'),
	    	'table' => 'tm_merk',
	    	'where' => array('tm_merk.is_delete' => 1)
	    );
	...
	public function list($keyword = '',$is_archive = '')
	{
		...
		if($keyword == 'none' OR $keyword == ''){
			...
			$like = array('tm_merk.kode' => $keyword);
			$or_like = array('tm_merk.nama' => $keyword);
		}
		$option1 = array(
			'select' => array(
								'tm_merk.merk_id',
	    	...
	    	'table' => 'tm_merk',
	    	'where_in' => array(
							'tm_merk.is_delete' => $is_archive,
						),
	    	'like' => $like,
	    	'or_like' => $or_like
	    );
	    $option2 = array(
			'select' => array('count(tm_merk.merk_id) as total'),
	    	'table' => 'tm_merk',
	    	'where_in' => array(
							'tm_merk.is_delete' => $is_archive,
						),
		...
		$this->load->view('admin/demo/ecommerce_merk',$data);
		
	}
	public function action_merk()
	{
		...
		$chk_name = array('tm_merk.kode' => $kode);
	...
	{
		$option = array(
			'select' => array('count(tm_merk.merk_id) as hasil'),
	    	'table' => 'tm_merk',
	    	'where' => $kondisi
	    );
	...
	public function get_edit_data()
	{
		$option = array(
	    	...
	    	'table' => 'tm_merk',
	    	'where' => array(
							'tm_merk.merk_id' => $this->input->post('id_merk')
						)
	    );
		...
		if( is_array($data['get_data']) || is_object($data['get_data']) ) {
		    foreach($data['get_data'] as $right) {
		    	echo json_encode(array(
					...
					"merk_kode" => $right->kode,
					"merk_nama" => $right->nama,
	...
	public function print_pdf($keyword = '',$is_archive = '')
	{
		...
		$pdf->Cell($tableX);
		$pdf->Cell($tableWidth, 10, 'DAFTAR MERK OHH_BAKERY', 0, 1, 'C');
		$pdf->Cell(10, 7, '', 0, 1);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell($tableWidth * 0.05, 6, 'No', 1, 0, 'C');
		$pdf->Cell($tableWidth * 0.35, 6, 'Kode Merk', 1, 0, 'C');
		$pdf->Cell($tableWidth * 0.60, 6, 'Nama Merk', 1, 1, 'C');
	...
	public function print_csv($keyword = '',$is_archive = '')
	{
		...
		$header = array("Kode Merk","Nama Merk"); 
		fputcsv($file, $header);
		foreach ($data as $key){ 
	...
	public function print_exc($keyword = '',$is_archive = '')
	{
        ...
        $sheet = $spreadsheet->getActiveSheet();
       	$sheet->setCellValue('A1', 'Kode Merk');
        $sheet->setCellValue('B1', 'Nama Merk');   
        $rows = 2;
        foreach ($data as $val){
