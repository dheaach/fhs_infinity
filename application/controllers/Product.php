...
class Product extends CI_Controller {
	...
	public function page($keyword = '',$is_archive = '', $group_id = '', $kat_id= '',$varian_id = '',$merk_id = '',$rak_id = '',$rak_gudang = '',$sts1 = '')
	{  
		if($is_archive <> ''){
			if($is_archive <> 'false'){
	    ...
	    $option6 = array(
	    	...
	    	'order' => array('tm_rak_gudang.nama' => 'ASC')
	    );
	    ...
	    $data['var'] =  $this->setting_model->commonGet($option3);
	    $data['merk'] =  $this->setting_model->commonGet($option4);
	    $data['rak'] =  $this->setting_model->commonGet($option5);
	    $data['rak_gud'] =  $this->setting_model->commonGet($option6);
	...
	//tab produk
	public function list($keyword = '',$is_archive = '', $group_id = '', $kat_id= '',$varian_id = '',$merk_id = '',$rak_id = '',$rak_gudang = '',$sts1 = '')
	{
		...
		if($varian_id == 0){
			...
			$where['tproduct.varian_id'] = $varian_id;
		}

		if($merk_id == 0){

		}else{
			$where['tproduct.merk_id'] = $merk_id;
		}

		if($rak_id == 0){

		}else{
			$where['tproduct.rak_id'] = $rak_id;
		}

		if($rak_gudang == 0){

		}else{
			$where['tproduct.rak_gudang'] = $rak_gudang;
		}
		...
		$option1 = array(
			'select' => array(
								...
								'tvarian.nama as nama_varian',
								'tm_merk.nama as nama_merk',
								'tm_rak.nama as nama_rak',
								'tm_rak_gudang.nama as nama_rakwh',
	    	...
	    	'join' => array(
	    					...
	    					'tvarian' => 'tvarian.varian_id = tproduct.varian_id',
	    					'tm_merk' => 'tm_merk.merk_id = tproduct.merk_id',
	    					'tm_rak' => 'tm_rak.rak_id = tproduct.rak_id',
	    					'tm_rak_gudang' => 'tm_rak_gudang.rak_gudang = tproduct.rak_gudang',
		...
		if (is_array($data['get_data']) || is_object($data['get_data'])){
		    foreach($data['get_data'] as $right) {
		    	$hasil['parent_prod'] = array(
					...
					"merk_id" => $right->merk_id,
					"rak_id" => $right->rak_id,
					"rak_gudang" => $right->rak_gudang,
