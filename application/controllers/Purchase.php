...
class Purchase extends CI_Controller {
	...
	public function order($keyword = '', $is_date = '', $start_date = '', $end_date = '', $status = '', $type = '', $is_all = '',$user_id = '')
	{
		...
		$select2 = array(
	    	...
	    	'where' => array(
							'tusers.is_delete' => 0),
	    	'order' => array('tusers.user_name' => 'ASC')
	    );

	    $ct1 = array(
			'select' => array('count(tpurchase_order.pur_ord) as open'),
	    	'table' => 'tpurchase_order',
	    	'where' => array(
							'tpurchase_order.is_delete' => 0,
							'tpurchase_order.statusOrder' => 0)
	    );
	    $ct2 = array(
			'select' => array('count(tpurchase_order.pur_ord) as progres'),
	    	'table' => 'tpurchase_order',
	    	'where' => array(
							'tpurchase_order.is_delete' => 0,
							'tpurchase_order.statusOrder' => 1
						)
	    );
	    $ct3 = array(
			'select' => array('count(tpurchase_order.pur_ord) as komplit'),
	    	'table' => 'tpurchase_order',
	    	'where' => array(
							'tpurchase_order.is_delete' => 0,
							'tpurchase_order.statusOrder' => 2
						)
	    );
	    $ct4 = array(
			'select' => array('count(tpurchase_order.pur_ord) as close'),
	    	'table' => 'tpurchase_order',
	    	'where' => array(
							'tpurchase_order.is_delete' => 0,
							'tpurchase_order.statusOrder' => 3
						)
	    );

	    $select3 = array(
			'select' => array('top_id', 'kode', 'ndays'),
	    	'table' => 'ttop',
	    	'where' => array('is_delete' => 0),
	    	'order' => array('is_default' => 'DESC')
	    );
		
		$data['user'] = $this->setting_model->commonGet($select2);
		$data['term'] = $this->setting_model->commonGet($select3);
		$data['open'] =  $this->setting_model->commonGet($ct1);
	    $data['progres'] =  $this->setting_model->commonGet($ct2);
	    $data['komplit'] =  $this->setting_model->commonGet($ct3);
	    $data['close'] =  $this->setting_model->commonGet($ct4);

		$this->load->view('admin/purchase_order',$data);
		
	}
	public function list_order($keyword = '', $is_date = '', $start_date = '', $end_date = '', $status = '', $type = '', $is_all = '',$user_id = '')
	{
		if($user_id == ''){
			$user_id = $this->session->userdata('person_id');
		}
		...
		if($keyword == 'none' OR $keyword == ''){
			...
			$keyword = str_replace("_"," ",$keyword);
			$like = array('a.pur_ord' => $keyword);
			$or_like = array('b.person_code' => $keyword,
								'b.person_name' => $keyword);
		}
		...
		if($is_date == 1 || $is_date == true){
			...
			$wtgl = array('a.pur_ord_date >=' => ($tgl_mulai.' 00:00:00'),
							'a.pur_ord_date <=' => ($tgl_sls.' 23:59:59'));
		}

		if($is_all == 0 || $is_all == false){
			$wuser = array('a.user_id' => $user_id);
		}

		if($status > 0){
			$wsts = array('a.statusOrder' => $status-1);	
		}
		...
		$option1 = array(
			'select' => array('a.*',
							...
							'ifnull(t2.user_name,"-"") as user_last_edit', 
							'c.kode as kode_cur'),
	    	'table' => 'tpurchase_order a',
	    	'join' => array('tperson b' => 'a.person_no = b.person_no',
	    					'tusers t1' => 'a.user_id = t1.user_id',
	    					'tusers t2' => 'a.user_edit = t2.user_id',
	    					'tusers t3' => 'a.user_delete = t3.user_id',
	    					'tm_uang c' => 'a.uang_id = c.uang_id'),
	    	'where' => $where,
	    	'where_in' => array(
							'a.is_delete' => $type
						),
	    	'like' => $like,
	    	'or_like' => $or_like,
	    	'order' => array('a.pur_ord_date' => 'DESC')
	    );

	    $option2 = array(
			'select' => array('count(a.pur_ord) as total'),
	    	'table' => 'tpurchase_order a',
	    	'join' => array('tperson b' => 'a.person_no = b.person_no',
	    					'tusers t1' => 'a.user_id = t1.user_id',
	    					'tusers t2' => 'a.user_edit = t2.user_id',
		...
		$data = array();
		$data['po'] = $this->setting_model->commonGet($option1);
		$data['tot'] = $this->setting_model->commonGet($option2);

		$this->load->view('admin/demo/purchase_order',$data);
	}

	public function show_list_supplier()
	{
		$key = $this->input->post('searchField');
		$type = $this->input->post('personType');
		...
		if($search == 'none' OR $search == ''){
			...
			$like = array('person_code' => $search);
			$or_like = array('person_name' => $search,
							'person_alamat' => $search,
							'person_telp' => $search,
		...
		$option1 = array(
			'select' => array('*'),
	    	'table' => 'tperson',
	    	'where' => array('is_delete' => 0, 'person_type' => $type),
	    	'like' => $like,
	    	'or_like' => $or_like,
	    	'order' => array('person_code' => 'ASC')
	    );

	    $option2 = array(
			'select' => array('count(person_no) as total'),
	    	'table' => 'tperson',
	    	'where' => array('is_delete' => 0, 'person_type' => $type),
	    	'like' => $like,
	    	'or_like' => $or_like,
		...
		$data['tot'] = $this->setting_model->commonGet($option2);
		$this->load->view('admin/demo/list_supplier',$data);
	}

	public function getFooterData()
	{
		$proses_id = $this->session->userdata('po_proses_id');
	...
	public function insert_detailpo()
	{
		$id_proses = 0;

		$id_proses = (int)$this->session->userdata('po_proses_id');

		if($id_proses <> 0 or $id_proses <> '' ){
			$id_proses = (int)$this->session->userdata('po_proses_id');
		}else{
			$id_proses = $this->get_id_proses(1,'po_proses_id');
		}
	    ...
	    $id_det = (int)$this->get_id_det_trans(6,$id_proses);
			...
			if (is_array($get_prod) || is_object($get_prod)){
				...
				foreach ($get_prod as $key) {
				    $dsn[] = array(
						        ...
						        "harga" => $key->prod_last_buy_price * $key->konversi1,
						        "harga_kurs" => $key->prod_last_buy_price * $key->konversi1,
						        "price_netto" => 0,
						        "price_netto_kurs" => 0,
						        "keterangan" => "",
						        "disc1_persen"=> 0,
								...
								"qty_order" => 0,
								"pur_ord_det_total" => 0,
								"pur_ord_det_total_kurs" => 0,
								"uang_id" => " ",
								"kurs_cur" => " ",
								"is_ppn" => $key->prod_tax_included,
								"ppn_persen" => " ",
								"disc_faktur_persen" => " ",
								"sub_total" => 0,
								"total" => 0,
	...
	public function fetch_detail_po()
	{
		$data_json = file_get_contents('./transaction_data.json');
					...
					if($val['id_trans'] == ((int)$proses_id)){
						foreach ($val['detail_trans'] as &$brg) {
					    	...
					    	foreach ($sc as $abc) {
					    		...
					    		$uom3 = $abc->prod_uom3;
					    		if($abc->prod_uom <> ''){
					    			$option .= '<option '.(((float)$brg['satuan'] == 1)? "selected='selected'" : '').' value="1">'.$abc->prod_uom.'</option>';
					    		}
					    		if($abc->prod_uom2 <> ''){
					    			$option .= '<option '.(((float)$brg['satuan'] == 2)? "selected='selected'" : '').' value="2">'.$abc->prod_uom2.'</option>';
					    		}
					    		if($abc->prod_uom3 <> ''){
					    			$option .= '<option '.(((float)$brg['satuan'] == 3)? "selected='selected'" : '').' value="3">'.$abc->prod_uom3.'</option>';
					    		}
							...
							$sub_array[] = '<div><select data-id="'.$brg['id_det'].'" data-column="satuan" data-value="' . $brg['satuan'] . '" data-uom="'.$uom.'" data-uom2="'.$uom2.'" data-uom3="'.$uom3.'" data-kon1="'.$kon1.'" data-kon2="'.$kon2.'" data-kon3="'.$kon3.'" data-konversi="'.$brg['konversi'].'" class="sel_satuan">'.$option.'</select></div>';
							...
							$sub_array[] = '<div contenteditable class="update-harga" data-id="'.$brg['id_det'].'" data-column="harga" data-value="' . $brg['harga'] . '">' . $brg['harga'] . '</div>';
							$sub_array[] = '<div data-id="'.$brg['id_det'].'" data-column="sub_total" data-value="' . $brg['sub_total'] . '">' . $brg['sub_total'] . '</div>';
							$sub_array[] = '<div contenteditable class="update-disc1-persen" data-id="'.$brg['id_det'].'" data-column="disc1_persen" data-value="' . $brg['disc1_persen'] . '">' . $brg['disc1_persen'] . '</div>';
							$sub_array[] = '<div contenteditable class="update-disc1-rp" data-id="'.$brg['id_det'].'" data-column="disc1_rp" data-value="' . $brg['disc1_rp'] . '">' . $brg['disc1_rp'] . '</div>';
							$sub_array[] = '<div contenteditable class="update-disc2-persen" data-id="'.$brg['id_det'].'" data-column="disc2_persen" data-value="' . $brg['disc2_persen'] . '">' . $brg['disc2_persen'] . '</div>';
							$sub_array[] = '<div contenteditable class="update-disc2-rp" data-id="'.$brg['id_det'].'" data-column="disc2_rp" data-value="' . $brg['disc2_rp'] . '">' . $brg['disc2_rp'] . '</div>';
							$sub_array[] = '<div contenteditable class="update-disc3-persen" data-id="'.$brg['id_det'].'" data-column="disc3_persen" data-value="' . $brg['disc3_persen'] . '">' . $brg['disc3_persen'] . '</div>';
							$sub_array[] = '<div contenteditable class="update-disc3-rp" data-id="'.$brg['id_det'].'" data-column="disc3_rp" data-value="' . $brg['disc3_rp'] . '">' . $brg['disc3_rp'] . '</div>';
							$sub_array[] = '<div data-id="'.$brg['id_det'].'" data-column="total" data-value="' . $brg['total'] . '">' . $brg['total'] . '</div>';
							$sub_array[] = '<div contenteditable class="update-ket" data-id="'.$brg['id_det'].'" data-column="keterangan" data-value="' . $brg['keterangan'] . '">' . $brg['keterangan'] . '</div>';
	...
	public function add_detail_po()
	{
		$proses_id = (int)$this->session->userdata('po_proses_id');
		...
		if(is_array($tableData)){
			foreach ($tableData as $data) {
				...
				$harga = $data['harga'];
				$harga_kurs = $data['harga_kurs'];
				$sub_total = $data['sub_total'];
				$disc1_persen = $data['disc1_persen'];
				$disc1_rp = $data['disc1_rp'];
				$disc2_persen = $data['disc2_persen'];
				$disc2_rp = $data['disc2_rp'];
				$disc3_persen = $data['disc3_persen'];
				$disc3_rp = $data['disc3_rp'];
				$total = $data['total'];
			    			...
			    			if($val['id_trans'] == ((int)$proses_id)){
								foreach ($val['detail_trans'] as &$brg) {
									if($brg['id_det'] == $id_det){
				    					...
				    					$brg['konversi'] = $kon;
				    					$brg['harga'] = $harga;
				    					$brg['harga_kurs'] = $harga_kurs;
				    					$brg['sub_total'] = $sub_total;
				    					$brg['disc1_persen'] = $disc1_persen;
				    					$brg['disc1_rp'] = $disc1_rp;
				    					$brg['disc2_persen'] = $disc2_persen;
				    					$brg['disc2_rp'] = $disc2_rp;
				    					$brg['disc3_persen'] = $disc3_persen;
				    					$brg['disc3_rp'] = $disc3_rp;
				    					$brg['total'] = $total;
	...
	public function delete_po_detail($id_det = "")
	{
		if($id_det == ''){
  ...
  //       	if($key['id'] ==6){
	...
	public function getProsesPo()
	{
		$this->session->unset_userdata('po_proses_id');
		$proses_id = $this->get_id_proses(6,'po_proses_id');
	...
	public function delete_all_detail_po($pr_id = '')
	{
		if($pr_id == ''){
			$pr_id = $this->input->post('proses_id');
			if($pr_id == ''){
				$pr_id = (int)$this->session->userdata('po_proses_id');
			}
		}
	...
	public function action_po()
	{
		...
		$id_tr = $this->session->userdata('po_proses_id');
		$user_id = $this->session->userdata('person_id');
		$user_cab = $this->session->userdata('gud_no');
		$now = date('Y-m-d H:i:s');
		$RekDasar = $this->setting_model->GetRekDasar();
		$rek_tax = $this->setting_model->ControlRek($RekDasar['PREFIX_NO_ORDER_BELI']);
		$uang_id = $this->setting_model->GetUang('uang_id');
		$kurs = $this->setting_model->GetUang('kurs');
		$ndays = $this->setting_model->GetTerm('ndays', $term);
		...
		if($this->input->post('type')==1){// insert
			if ($is_bb == 1) {
				...
				$this->delete_all_detail_po($id_tr);
			}else{

				$insert = array(
					'insert' => array(
						'cab_no' => $user_cab,
						'prefix_no_tax' => $rek_tax, 
						'create_date' => $now, 
						'edit_date' => $now, 
						'user_edit' => $user_id, 
						'user_id' => $user_id,
						...
						'top_id' => $term,
						'ndays' => $ndays,
						'pur_ord_subtotal' => $sub_total, 
						'pur_ord_subtotal_kurs' => $sub_total, 
						'pur_ord_pot_persen' => $disc_persen, 
						'pur_ord_pot_rp' => $disc_rp, 
						'pur_ord_pot_rp_kurs' => $disc_rp, 
						'pur_ord_ppn_persen' => $ppn, 
						'pur_ord_ppn_rp' => $ppn_rp, 
						'pur_ord_ppn_rp_kurs' => $ppn_rp, 
						'is_ppn' => $ppn_type,
						'pur_ord_total' => $total, 
						'pur_ord_total_kurs' => $total, 
						'tax_amount' => $ppn,
						'is_delete' => 0
					),
					'table' => 'tset_bb'
	        	...
	        	$this->setting_model->commonInsert($insert);
	        	$this->insert_detail_po($id_tr, $trans_no);
        	...
        	$this->setting_model->commonUpdate($update);
        	$this->update_detail_po($id_tr, $id_bb);
	...
	public function get_edit_data_bb()
	{
		...
		$proses_id = (int)$this->getProsesPo();
		$pid = $proses_id;
		...
		$id_det = (int)$this->get_id_det_trans(6,$proses_id);
	...
	public function update_po_change()
	{
		$id = $this->input->post('id');
		$column = $this->input->post('column');
	    $vl = $this->input->post('value');
        	...
        	if($key['id'] == 6){
				foreach ($key['detail'] as &$val) {
					if($val['id_trans'] == ((int)$proses_id)){
						foreach ($val['detail_trans'] as &$brg) {
							if($brg['id_det'] == ((int)$id)){
								$oldKonv = $brg['konversi'];

								if($column == "harga"){
									$brg["harga_kurs"] = (float)$vl;
									$vl = (float)$vl;
								}
								...
								if($column == "konversi"){
									if($brg["harga"] > 0){
										if((float)$vl > (float)$oldKonv){
											$brg["harga"] = ((float)$vl * (float)$brg['harga']);
											$brg["harga_kurs"] = (float)$brg['harga'];
										}elseif ((float)$vl < (float)$oldKonv) {
											$brg["harga"] = ((float)$brg['harga'] / (float)$oldKonv);
											$brg["harga_kurs"] = (float)$brg['harga'];
										}	
									}
								}
								
								$brg["sub_total"] = ((float)$brg['qty_satuan'] * (float)$brg['harga_kurs']);
								
								if($column == "disc1_persen" or $column == "disc2_persen" or $column == "disc3_persen"){
									if($brg['harga_kurs'] > 0){
										$brg['disc1_rp'] = ((float)$brg['disc1_persen'] / 100) * (float)$brg['sub_total']; 
										$disc1 = (float)$brg['sub_total'] - $brg['disc1_rp'];
										$brg['disc2_rp'] = ((float)$brg['disc2_persen'] / 100) * (float)$disc1; 
										$disc2 = (float)$brg['sub_total'] - $brg['disc1_rp'] - $brg['disc2_rp'];
										$brg['disc3_rp'] = ((float)$brg['disc3_persen'] / 100) * (float)$disc2; 
									}
	            					
								}else if($column = "disc1_rp" or $column = "disc2_rp" or $column = "disc3_rp"){
									if($brg['harga_kurs'] > 0){
										$brg['disc1_persen'] = round(((float) $brg['disc1_rp'] / (float)$brg['sub_total']) * 100 , 5); 
										$disc1 = (float)$brg['sub_total'] - $brg['disc1_rp'];
										$brg['disc2_persen'] = round(((float) $brg['disc2_rp'] / $disc1) * 100 , 5); 
										$disc2 = (float)$brg['sub_total'] - $brg['disc1_rp'] - $brg['disc2_rp'];
										$brg['disc3_persen'] = round(((float) $brg['disc3_rp'] / $disc2) * 100 , 5);
									}
									
								}

								$afterDisc = (float)$brg["sub_total"] - $brg['disc1_rp'] - $brg['disc2_rp'] - $brg['disc3_rp'];
  ...
  //       $this->pdf->createPDF($html, $id_bb.'_'.date('Ymd'), false);
	...
	public function getPerson()
	{
		...
		$person_name = '';
		$select = array(
			'select' => array('person_no', 'person_code', 'person_name'),
	    	'table' => 'tperson',
	    	'where' => array('person_no' => $id_person)
	    );

		$person = $this->setting_model->commonGet($select);
