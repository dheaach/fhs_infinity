...
class Rak_wh extends CI_Controller {
	...
	{  
		...
		$ct1 = array(
			'select' => array('count(tm_rak_gudang.rak_gudang) as aktif'),
	    	'table' => 'tm_rak_gudang',
	    	'where' => array(
							'tm_rak_gudang.is_delete' => 0)
	    );
	    $ct2 = array(
			'select' => array('count(tm_rak_gudang.rak_gudang) as non'),
	    	'table' => 'tm_rak_gudang',
	    	'where' => array('tm_rak_gudang.is_delete' => 1)
	    );
		...
		$this->load->view("admin/rak_wh",$data);
	}
	...
	public function list($keyword = '',$is_archive = '')
	{
		...
		if($keyword == 'none' OR $keyword == ''){
			...
			$like = array('tm_rak_gudang.kode' => $keyword);
			$or_like = array('tm_rak_gudang.nama' => $keyword);
		}
		$option1 = array(
			'select' => array(
								'tm_rak_gudang.rak_gudang',
								'tm_rak_gudang.kode',
								'tm_rak_gudang.nama'
							),
	    	'table' => 'tm_rak_gudang',
	    	'where_in' => array(
							'tm_rak_gudang.is_delete' => $is_archive,
						),
	    	'like' => $like,
	    	'or_like' => $or_like
	    );
	    $option2 = array(
			'select' => array('count(tm_rak_gudang.rak_gudang) as total'),
	    	'table' => 'tm_rak_gudang',
	    	'where_in' => array(
							'tm_rak_gudang.is_delete' => $is_archive,
						),
		...
		$this->load->view('admin/demo/ecommerce_rakwh',$data);
		
	}
	public function action_rakwh()
	{
		$id_rakwh =$this->input->post('id_rakwh');
		$kode=$this->input->post('kode');
		$nama=$this->input->post('nama');
		$status=$this->input->post('status');

		$chk_name = array('tm_rak_gudang.kode' => $kode);
		...
		if($this->input->post('type')==1){// insert
			...
			$update = array(
					...
					'table' => 'tm_rak_gudang',
					'where' => array(
						'tm_rak_gudang.rak_gudang' => $id_rakwh
					)
			);
			...
			echo json_encode(array(
			 ...
			 foreach($id_rakwh as $id){
				$update = array(
						'update' => array(
							'is_delete' => 1),
						'table' => 'tm_rak_gudang',
						'where' => array(
							'tm_rak_gudang.rak_gudang' => $id
						)
				);
	        	$this->setting_model->commonUpdate($update);
		     }
	...
	{
		$option = array(
			'select' => array('count(tm_rak_gudang.rak_gudang) as hasil'),
	    	'table' => 'tm_rak_gudang',
	    	'where' => $kondisi
	    );
	...
	public function get_edit_data()
	{
		$option = array(
			'select' => array(
								...
								'tm_rak_gudang.kode',
								'tm_rak_gudang.nama',
								'tm_rak_gudang.is_delete',
							),
	    	'table' => 'tm_rak_gudang',
	    	'where' => array(
							'tm_rak_gudang.rak_gudang' => $this->input->post('id_rakwh')
						)
	    );
		...
		if( is_array($data['get_data']) || is_object($data['get_data']) ) {
		    foreach($data['get_data'] as $right) {
		    	echo json_encode(array(
					"rakwh_gudang" => $right->rak_gudang,
					"rakwh_kode" => $right->kode,
					"rakwh_nama" => $right->nama,
	...
	public function print_pdf($keyword = '',$is_archive = '')
	{
		...
		$pdf->Cell($tableX);
		$pdf->Cell($tableWidth, 10, 'DAFTAR RAK-WH OHH_BAKERY', 0, 1, 'C');
		$pdf->Cell(10, 7, '', 0, 1);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell($tableWidth * 0.05, 6, 'No', 1, 0, 'C');
		$pdf->Cell($tableWidth * 0.35, 6, 'Kode Rak WH', 1, 0, 'C');
		$pdf->Cell($tableWidth * 0.60, 6, 'Nama Rak WH', 1, 1, 'C');
	...
	{
		// file name 
		$filename = 'rak-wh_'.date('d-m-Y').'.csv'; 
		...
		$header = array("Kode Rak WH","Nama Rak WH"); 
		fputcsv($file, $header);
		foreach ($data as $key){ 
	...
	public function print_exc($keyword = '',$is_archive = '')
	{
        ...
        $sheet = $spreadsheet->getActiveSheet();
       	$sheet->setCellValue('A1', 'Kode Rak WH');
        $sheet->setCellValue('B1', 'Nama Rak WH');   
        $rows = 2;
        foreach ($data as $val){
