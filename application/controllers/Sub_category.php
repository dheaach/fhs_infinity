...
class Sub_category extends CI_Controller {
	...
	public function action_subcategory()
	{
		...
		$chk_name = array('tgroup.kode' => $kode);
	...
	public function print_pdf($keyword = '',$is_archive = '')
	{
		...
		$pdf->Cell($tableX);
		$pdf->Cell($tableWidth, 10, 'DAFTAR VARIAN OHH_BAKERY', 0, 1, 'C');
		$pdf->Cell(10, 7, '', 0, 1);
		$pdf->SetFont('Arial', 'B', 8);
		$pdf->Cell($tableWidth * 0.05, 6, 'No', 1, 0, 'C');
		$pdf->Cell($tableWidth * 0.35, 6, 'Kode Varian', 1, 0, 'C');
		$pdf->Cell($tableWidth * 0.60, 6, 'Nama Varian', 1, 1, 'C');
	...
	public function print_csv($keyword = '',$is_archive = '')
	{
		...
		$header = array("Kode Varian","Nama Varian"); 
		fputcsv($file, $header);
		foreach ($data as $key){ 
	...
	public function print_exc($keyword = '',$is_archive = '')
	{
        ...
        $sheet = $spreadsheet->getActiveSheet();
       	$sheet->setCellValue('A1', 'Kode Varian');
        $sheet->setCellValue('B1', 'Nama Varian');   
        $rows = 2;
        foreach ($data as $val){
