			...
			<div class="page-bar">
				<div class="row">
					...
					<div class="add-sub-cat-nav col-sm-2">
						<a class="btn btn-sm green-seagreen" data-toggle="modal" id="55B"><i class="fa fa-plus"></i> Customer Baru</a>
						<input type="hidden" id="id_page" value="55">
					</div>
				</div>
							...
							<div class="tabbable-line">
								<div class="tab-content">
									<div class="tab-pane active" id="tab_15_1">
											...
											<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 margin-bottom-20">
												<div class="dashboard-stat db-table">
													...
													<div class="details db-lbl ">
														...
														<div class="desc db-desc">
															 <strong>Customer Non-Aktif</strong>
														</div>
													</div>
												</div>
											</div>
										...
										<div class="row">
											<div class="col-sm-6">
												...
												<div class="btn-group" id="btn_act" style="display:none;">
													<a class="btn green-seagreen dropdown-toggle btn-rounded btn-sm btn-border btn-actcl" data-toggle="dropdown" href="#" id="55D">
													Aksi
													</a>
											...
											<div class="col-sm-3 pdg-btn">
												<div class="btn-group">
													<a class="btn green-seagreen btn-sm btn-rounded-left btn-border" id="55E">
														Import
													</a>
													<div class="btn-group">
														<a class="btn green-seagreen dropdown-toggle btn-rounded-right btn-sm btn-border" data-toggle="dropdown" href="#" id="55F">
														Eksport
														</a>
											...
											<div class="col-sm-3 pdg-search">
												<div class="input-group">
													...
													<div class="input-group-btn">
														<button type="button" id="search-btn-customer" class="btn btn-default advance-toggle  btn-rounded-right"><i class="fa fa-angle-down" id="search-icn-customer"></i></button>
														
														<div class="advance-search-toggle" id="search-toggle-customer" style="min-width:250px;display: none;">
															<div class="row">
																<div class="col-sm-12" style="margin:12px 5px 15px;padding-right: 23px;">
																	...
																	<div class="form-group">
																		<div class="col-sm-6" style="margin: 30px 0px 0px 0px;padding: 3px 8px 0px 15px;text-align: right;">
																			<a class="" style="color:#777;text-decoration:none;" id="btn-rf-customer">Hapus Filter</a>
																		</div>
																		<div class="col-sm-6" style="margin: 30px 0px 0px 0px;color: #777;padding: 0px 15px 0px 0px;">
																			<a class="btn green-seagreen btn-rounded" style="display:block;" id="btn-sf-customer">
																				Cari
																			</a>
																		</div>
																	</div>
										...
										<div class="tab-content" id="55A" style="display:none;">
											<div class="tab-pane fade active in" id="tab_2_1">
												<div class="table-scrollable" style="border: 0px solid #dddddd;">
													<div class="table-actions-wrapper">
													</div>
													<table class="table table-hover" id="datatable_customer">
...
<script>
        ...
        var countCheckedCustomer = function($table, checkboxClass) {
		  if ($table) {
        ...
        function checkCustomer() {
		  var result = countCheckedCustomer($('#datatable_customer'), '.chk-customer');
		...
		function calc()
		{
			$('.checkall').click(function (event) {    
		        $('.chk-customer').prop('checked', this.checked);
		        var $checkboxes = $('.chk-customer');
		...
		{
		    var container = $("#search-toggle-customer");
		    var btn = $("#search-btn-customer");
		    var icn = $("#search-icn-customer");
		    var x = document.getElementById("search-toggle-customer");

		    // if the target of the click isn't the container nor a descendant of the container
		    if (!container.is(e.target) && container.has(e.target).length === 0 && !btn.is(e.target) && !icn.is(e.target)) 
		    {
		        $("#search-toggle-customer").hide();
		    }else if(btn.is(e.target) || icn.is(e.target)){
		    	
				if (x.style.display === "none") {
				  $("#search-toggle-customer").show();
				} else {
				  $("#search-toggle-customer").hide();
				}
		    }
		...
		function save_customer() {
			...
			var is_except_term = $('#is_except_term').prop('checked');
			var jenis_harga = $('#jenis_harga').val();
			var is_person_limit = $('#is_person_limit').prop('checked');
			var person_limit_piutang = $('#person_limit_piutang').val();
			...
			is_except_term = (is_except_term == true) ? 1 : 0;
			is_person_limit = (is_person_limit == true) ? 1 : 0;
			...
			if(person_name!="" &&  (is_active !="" || is_non !="")){
				...
				$.ajax({
					url: "<?php echo base_url("customer/action_customer");?>",
					type: "POST",
					data: {
						type: type,
						id_person : id_person,
						...
						is_except_term :is_except_term,
						jenis_harga: jenis_harga,
						is_person_limit : is_person_limit,
						person_limit_piutang : person_limit_piutang,
						salesman : salesman,
						status : status
					},
					cache: false,
				...
				if (person_name =="") {
					...
					$('#validation-supplier').html('Nama Customer harus diisi!');
		...
		$('#btn-s-supplier').on('click', function(e) {
			...
			if(person_name!="" &&  (is_active !="" || is_non !="" )){
				var oTable = $('#datatable_customer').DataTable();
				oTable.draw();
		...
		$('#55F').on('click', function() {
			if(check_user_right(this.id) == 0){
				$('#modal_unauthorized').modal('show');
			}			
		});
		$('#55E').on('click', function() {
			if(check_user_right(this.id) == 0){
				$('#modal_unauthorized').modal('show');
			}			
		});
		$('#55D').on('click', function() {
			if(check_user_right(this.id) == 0){
				$('#modal_unauthorized').modal('show');
			}			
		});
		// action edit clicked row
		function cae_customer(id_person) {
			var x = document.getElementById("55C");
	...
	            $.ajax({
					url: "<?php echo base_url("customer/get_edit_data");?>",
					type: "POST",
					data: {
						id_person: id_person
					},
					...
					success: function(dataResult){
						...
						$('#is_except_term').prop('checked',json_data.is_except_term);
						$('#jenis_harga').val(json_data.jenis_harga).trigger('change');
						$('#is_person_limit').prop('checked',json_data.is_person_limit);
						$('#person_limit_piutang').val(json_data.person_limit_piutang);
						$('#salesman').val(json_data.salesman).trigger('change');
		...
		$('#55B').on('click', function() {
			var id_page = $('#id_page').val();

			if(check_user_right(this.id) == 0){
				$('#modal_unauthorized').modal('show');
			}else{
				if(id_page == '55' || id_page == 55){
					...
					$.ajax({
						url: "<?php echo base_url("customer/get_rek_dasar");?>",
						type: "POST",
						cache: false,
						success: function(dataResult){
							// console.log(dataResult);
		...
		$('#btn-h-akses').on('click', function() {
			// Get userid from checked checkboxes
				...
				$(".chk-customer:checked").each(function(){
					...
					customer_arr.push(varid);
				});

				// Array length
				var length = customer_arr.length;
		...
		function search_customer() {
			...
			var urlist = "<?php echo base_url('customer/page');?>";
		...
		$('#btn-rf-customer').on('click', function(e) {
		...
		//action btn cari
		$('#btn-sf-customer').on('click', function(e) {
			search_customer();
		});
		...
		$(document).on('click', '#btn-pdf-grp', function(e){
			...
			var is_ar = $("#chk-arsip").prop("checked");
			var urlist = "<?php echo base_url('customer/print_pdf');?>";
		...
		$(document).on('click', '#btn-csv-grp', function(e){
			...
			var is_ar = $("#chk-arsip").prop("checked");
			var urlist = "<?php echo base_url('customer/print_csv');?>";
		...
		$(document).on('click', '#btn-exc-grp', function(e){
			...
			var is_ar = $("#chk-arsip").prop("checked");
			var urlist = "<?php echo base_url('customer/print_exc');?>";
