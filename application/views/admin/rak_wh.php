		...
		<div class="page-content">
			...
			<div class="page-bar">
				<div class="row">
					<div class="col-sm-9">
						...
						<strong>Daftar Rak Warehouse</strong>
						</h3>
					</div>
					<div class="add-subcat-nav col-sm-3">
						<a class="btn btn-sm green-seagreen" data-toggle="modal" id="27B"><i class="fa fa-plus"></i> Rak-Wh Baru</a>
					</div>
				</div>
			...
			<div class="page-bar pg-breadcrumb">
				<ul class="page-breadcrumb">
					...
					<li>
						<a href="<?php echo base_url('rak_wh');?>">Master Rak Warehouse</a>
					</li>
				</ul>
			</div>
							...
							<div class="tabbable-line">
								<div class="tab-content">
									<div class="tab-pane active" id="tab_15_1">
												...
												<div class="dashboard-stat db-table">
													...
													<div class="details db-lbl ">
														...
														<div class="desc db-desc">
															 <strong>Rak Warehouse Aktif</strong>
														</div>
													</div>
												</div>
											</div>
											<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 margin-bottom-20">
												<div class="dashboard-stat db-table">
													...
													<div class="details db-lbl ">
														...
														<div class="desc db-desc">
															 <strong>Rak Warehouse Non-Aktif</strong>
														</div>
													</div>
												</div>
											</div>
										...
										<div class="row">
											<div class="col-sm-6">
												...
												<div class="btn-group" id="btn_act" style="display:none;">
													<a class="btn green-seagreen dropdown-toggle btn-rounded btn-sm btn-border btn-actcl" data-toggle="dropdown" href="#" id="27D">
													Aksi
													</a>
											...
											<div class="col-sm-3 pdg-btn">
												<div class="btn-group">
													<a class="btn green-seagreen btn-sm btn-rounded-left btn-border" id="27E">
														Import
													</a>
													<div class="btn-group">
														<a class="btn green-seagreen dropdown-toggle btn-rounded-right btn-sm btn-border" data-toggle="dropdown" href="#" id="27F">
														Eksport
														</a>
											...
											<div class="col-sm-3 pdg-search">
												<div class="input-group">
													...
													<div class="input-group-btn">
														<button type="button" id="search-btn-rakwh" class="btn btn-default advance-toggle  btn-rounded-right"><i class="fa fa-angle-down" id="search-icn-rakwh"></i></button>
														
														<div class="advance-search-toggle" id="search-toggle-rakwh" style="min-width:250px;display: none;">
															<div class="row">
																<div class="col-sm-12" style="margin:12px 5px 15px;padding-right: 23px;">
																	...
																	<div class="form-group">
																		<div class="col-sm-6" style="margin: 30px 0px 0px 0px;padding: 3px 8px 0px 15px;text-align: right;">
																			<a class="" style="color:#777;text-decoration:none;" id="btn-rf-rakwh">Hapus Filter</a>
																		</div>
																		<div class="col-sm-6" style="margin: 30px 0px 0px 0px;color: #777;padding: 0px 15px 0px 0px;">
																			<a class="btn green-seagreen btn-rounded" style="display:block;" id="btn-sf-rakwh">
																				Cari
																			</a>
																		</div>
																	</div>
										...
										<div class="tab-content" id="27A" style="display:none;">
											<div class="tab-pane fade active in" id="tab_2_1">
												<div class="table-scrollable" style="border: 0px solid #dddddd;">
													<div class="table-actions-wrapper">
													</div>
													<table class="table table-hover" id="datatable_rakwh">
														<thead class="thead-dark">
															...
															<tr role="row" class="filter panel-collapse collapse" id="collapse_2">
																<td></td>
																<td>
																	<input type="text" class="form-control form-filter input-sm" name="rakwh_kode">
																</td>
																<td>
																	<input type="text" class="form-control form-filter input-sm" name="rakwh_name">
																</td>
															</tr>
														</thead>
														<tbody>
...
<script>
        ...
        var countCheckedRakWh = function($table, checkboxClass) {
		  if ($table) {
        ...
        function checkRakWh() {
		  var result = countCheckedRakWh($('#datatable_rakwh'), '.chk-rakwh');
		...
		function calc()
		{
			$('.checkall').click(function (event) {    
		        $('.chk-rakwh').prop('checked', this.checked);
		        var $checkboxes = $('.chk-rakwh');
		...
		{
		    var container = $("#search-toggle-rakwh");
		    var btn = $("#search-btn-rakwh");
		    var icn = $("#search-icn-rakwh");
		    var x = document.getElementById("search-toggle-rakwh");

		    // if the target of the click isn't the container nor a descendant of the container
		    if (!container.is(e.target) && container.has(e.target).length === 0 && !btn.is(e.target) && !icn.is(e.target)) 
		    {
		        $("#search-toggle-rakwh").hide();
		    }else if(btn.is(e.target) || icn.is(e.target)){
		    	
				if (x.style.display === "none") {
				  $("#search-toggle-rakwh").show();
				} else {
				  $("#search-toggle-rakwh").hide();
				}
		    }
		...
		function save_rakwh() {
			var id_rakwh = $('#id_rakwh').val();
			var type = 1;

			if(id_rakwh !=""){
				type = 2;
			}
			var kode = $('#kode_rakwh').val();
			var nama = $('#nama_rakwh').val();
			
			var is_active = $('#is_act_rakwh').prop('checked');
			var is_non = $('#is_non_rakwh').prop('checked');
			...
			if(kode!="" && nama!="" &&  (is_active !="" || is_non !="")){
				$("#btn-s-rakwh").attr("disabled", "disabled");
				$("#btn-rs-rakwh").attr("disabled", "disabled");
				$("#btn-r-rakwh").attr("disabled", "disabled");
				$.ajax({
					url: "<?php echo base_url("rak_wh/action_rakwh");?>",
					type: "POST",
					data: {
						type: type,
						id_rakwh : id_rakwh,
					...
					success: function(dataResult){
						...
						if(dataResult.statusCode==200){
							$("#btn-s-rakwh").removeAttr("disabled");
							$("#btn-rs-rakwh").removeAttr("disabled");
							$("#btn-r-rakwh").removeAttr("disabled");
							$('#frm-rakwh').find('input:text').val('');
							$("#success").show();
				...
				if(kode ==""){
					...
					$("#btn-rs-rakwh").removeAttr("disabled");
					$("#btn-r-rakwh").removeAttr("disabled");
					$("#validation-rakwh").show();
					$('#validation-rakwh').html('Kode rak harus diisi!');
					$("#validation-rakwh").fadeTo(2000, 500).slideUp(500, function() {
	                	$("#validation-rakwh").slideUp(500);
	                });	
				}else if (nama =="") {
					$("#btn-s-rakwh").removeAttr("disabled");
					$("#btn-rs-rakwh").removeAttr("disabled");
					$("#btn-r-rakwh").removeAttr("disabled");
					$("#validation-rakwh").show();
					$('#validation-rakwh').html('Nama rak harus diisi!');
					$("#validation-rakwh").fadeTo(2000, 500).slideUp(500, function() {
	                	$("#validation-rakwh").slideUp(500);
	                });	
				}else{
					...
					$("#btn-r-rakwh").removeAttr("disabled");
					$("#validation-rakwh").show();
					$('#validation-rakwh').html('Harap isi semua data yang dibutuhkan!');
					$("#validation-rakwh").fadeTo(2000, 500).slideUp(500, function() {
	                	$("#validation-rakwh").slideUp(500);
	                });
				}
		...
		//action btn save
		$('#btn-s-rakwh').on('click', function(e) {
			e.preventDefault();
			...
			if(kode!="" && nama!="" &&  (is_active !="" || is_non !="")){
				var oTable = $('#datatable_rakwh').DataTable();
				oTable.draw();

				$('#add_rakwh').modal('hide');
				$.uniform.update();	
			}else{
				if(kode ==""){
					$("#btn-s-rakwh").removeAttr("disabled");
		...
		$('#btn-rs-rakwh').on('click', function(e) {
			e.preventDefault();
			save_rakwh();
			if(is_valid != 0){
				$("#validation-s-rakwh").show();
				$('#validation-s-rakwh').html('Data berhasil ditambahkan!');
				$("#validation-s-rakwh").fadeTo(2000, 500).slideUp(500, function() {
                	$("#validation-s-rakwh").slideUp(500);
                });
			}
			var $t = $(this),
		...
		//action btn reset
		$('#btn-r-rakwh').on('click', function(e) {
			e.preventDefault();
			var $t = $(this),
			target = $t[0].href || $t.data("target") || $t.parents('.modal') || [];
		...
		$('#27F').on('click', function() {
			if(check_user_right(this.id) == 0){
				$('#modal_unauthorized').modal('show');
			}			
		});
		$('#27E').on('click', function() {
			if(check_user_right(this.id) == 0){
				$('#modal_unauthorized').modal('show');
			}			
		});
		$('#27D').on('click', function() {
			if(check_user_right(this.id) == 0){
				$('#modal_unauthorized').modal('show');
			}			
		});
		// action edit clicked row
		function cae_rakwh(id_rakwh) {
			var x = document.getElementById("27C");
			if(check_user_right(x.id) == 0){
				$('#modal_unauthorized').modal('show');
			}else{
	            // Set data to Form Edit
	            $('#id_rakwh').val(id_rakwh);
	            $.ajax({
					url: "<?php echo base_url("rak_wh/get_edit_data");?>",
					type: "POST",
					data: {
						id_rakwh: id_rakwh
					},
					...
					success: function(dataResult){
						...
						$('#id_rakwh').val(json_data.rakwh_id);
						$('#kode_rakwh').val(json_data.rakwh_kode);
						$('#nama_rakwh').val(json_data.rakwh_nama);

						var status = json_data.status;

						// console.log(status);
						if(status == 0 ){
							$('#is_act_rakwh').prop('checked', true);
							$('#is_non_rakwh').prop('checked', false);
						}else{
							$('#is_act_rakwh').prop('checked', false);
							$('#is_non_rakwh').prop('checked', true);
						}
				...
				$('#add_rakwh').modal('show');
			}
		}

		$('#27B').on('click', function() {
			if(check_user_right(this.id) == 0){
				$('#modal_unauthorized').modal('show');
			}else{
				$('#is_act_rakwh').attr('checked', true);
				$('#is_act_rakwh').closest('span').addClass('checked');
				$('#is_non_rakwh').attr('checked', false);
				$('#is_non_rakwh').closest('span').removeClass('checked');
				$('#add_rakwh').modal('show');
			}		
		});
		...
		$('#btn-h-akses').on('click', function() {
			// Get userid from checked checkboxes
				var rakwh_arr = [];
				$(".chk-rakwh:checked").each(function(){
					var varid = $(this).val();

					rakwh_arr.push(varid);
				});

				// Array length
				var length = rakwh_arr.length;
		...
		function search_rakwh() {
			...
			var urlist = "<?php echo base_url('rak_wh/page');?>";
		...
		$('#btn-rf-rakwh').on('click', function(e) {
		...
		//action btn cari
		$('#btn-sf-rakwh').on('click', function(e) {
			search_rakwh();
		});
		...
		$(document).on('click', '#btn-pdf-grp', function(e){
			...
			var is_ar = $("#chk-arsip").prop("checked");
			var urlist = "<?php echo base_url('rak_wh/print_pdf');?>";
		...
		$(document).on('click', '#btn-csv-grp', function(e){
			...
			var is_ar = $("#chk-arsip").prop("checked");
			var urlist = "<?php echo base_url('rak_wh/print_csv');?>";
		...
		$(document).on('click', '#btn-exc-grp', function(e){
			...
			var is_ar = $("#chk-arsip").prop("checked");
			var urlist = "<?php echo base_url('rak_wh/print_exc');?>";
		...
		$('#btn-b-rakwh').on('click', function(e) {
			e.preventDefault();
			var $t = $(this),
			target = $t[0].href || $t.data("target") || $t.parents('.modal') || [];
