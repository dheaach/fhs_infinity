		...
		<div class="page-content">
			...
			<div class="page-bar">
				<div class="row">
					<div class="col-sm-10">
						...
						<strong>Daftar Term</strong>
						</h3>
					</div>
					<div class="add-sub-cat-nav col-sm-2">
						<a class="btn btn-sm green-seagreen" data-toggle="modal" id="57B"><i class="fa fa-plus"></i> Term Baru</a>
					</div>
				</div>
			...
			<div class="page-bar pg-breadcrumb">
				<ul class="page-breadcrumb">
					...
					<li>
						<a href="<?php echo base_url('top');?>">Master Term</a>
					</li>
				</ul>
			</div>
							...
							<div class="tabbable-line">
								<div class="tab-content">
									<div class="tab-pane active" id="tab_15_1">
										...
										<div class="row">
											<div class="col-sm-6">
												...
												<div class="btn-group" id="btn_act" style="display:none;">
													<a class="btn green-seagreen dropdown-toggle btn-rounded btn-sm btn-border btn-actcl" data-toggle="dropdown" href="#" id="57D">
													Aksi
													</a>
											...
											<div class="col-sm-3 pdg-btn">
												<div class="btn-group">
													<a class="btn green-seagreen btn-sm btn-rounded-left btn-border" id="57E">
														Import
													</a>
													<div class="btn-group">
														<a class="btn green-seagreen dropdown-toggle btn-rounded-right btn-sm btn-border" data-toggle="dropdown" href="#" id="57F">
														Eksport
														</a>
											...
											<div class="col-sm-3 pdg-search">
												<div class="input-group">
													...
													<div class="input-group-btn">
														<button type="button" id="search-btn-top" class="btn btn-default advance-toggle  btn-rounded-right"><i class="fa fa-angle-down" id="search-icn-top"></i></button>
														
														<div class="advance-search-toggle" id="search-toggle-top" style="min-width:250px;display: none;">
															<div class="row">
																<div class="col-sm-12" style="margin:12px 5px 15px;padding-right: 23px;">
																	...
																	<div class="form-group">
																		<div class="col-sm-6" style="margin: 30px 0px 0px 0px;padding: 3px 8px 0px 15px;text-align: right;">
																			<a class="" style="color:#777;text-decoration:none;" id="btn-rf-top">Hapus Filter</a>
																		</div>
																		<div class="col-sm-6" style="margin: 30px 0px 0px 0px;color: #777;padding: 0px 15px 0px 0px;">
																			<a class="btn green-seagreen btn-rounded" style="display:block;" id="btn-sf-top">
																				Cari
																			</a>
																		</div>
																	</div>
										...
										<div class="tab-content" id="57A" style="display:none;">
											<div class="tab-pane fade active in" id="tab_2_1">
												<div class="table-scrollable" style="border: 0px solid #dddddd;">
													<div class="table-actions-wrapper">
													</div>
													<table class="table table-hover" id="datatable_top">
...
<script>
        ...
        var countCheckedTerm = function($table, checkboxClass) {
		  if ($table) {
        ...
        function checkTerm() {
		  var result = countCheckedTerm($('#datatable_top'), '.chk-top');
		...
		function calc()
		{
			$('.checkall').click(function (event) {    
		        $('.chk-top').prop('checked', this.checked);
		        var $checkboxes = $('.chk-top');
		...
		{
		    var container = $("#search-toggle-top");
		    var btn = $("#search-btn-top");
		    var icn = $("#search-icn-top");
		    var x = document.getElementById("search-toggle-top");

		    // if the target of the click isn't the container nor a descendant of the container
		    if (!container.is(e.target) && container.has(e.target).length === 0 && !btn.is(e.target) && !icn.is(e.target)) 
		    {
		        $("#search-toggle-top").hide();
		    }else if(btn.is(e.target) || icn.is(e.target)){
		    	
				if (x.style.display === "none") {
				  $("#search-toggle-top").show();
				} else {
				  $("#search-toggle-top").hide();
				}
		    }
		...
		function save_top() {
			...
			var top_code = $('#top_code').val();
			var term = $('#term').val();
			var is_default_term = $('#is_default_term').prop('checked');
			
			var is_active = $('#is_act_top').prop('checked');
			var is_non = $('#is_non_top').prop('checked');

			is_default_term = (is_default_term == true) ? 1 : 0;
			...
			if(term!="" && top_code!="" && (is_active !="" || is_non !="")){
				$("#btn-s-top").attr("disabled", "disabled");
				$("#btn-rs-top").attr("disabled", "disabled");
				$("#btn-r-top").attr("disabled", "disabled");
				$.ajax({
					url: "<?php echo base_url("top/action_top");?>",
					type: "POST",
					data: {
						type: type,
						id_top : id_top,
						top_code : top_code,
						term : term,
						is_default_term : is_default_term,
						status : status
					},
					cache: false,
					success: function(dataResult){
						...
						if(dataResult.statusCode==200){
							$("#btn-s-top").removeAttr("disabled");
							$("#btn-rs-top").removeAttr("disabled");
							$("#btn-r-top").removeAttr("disabled");
							$('#frm-top').find('input:text').val('');
							$("#success").show();
				...
				if (top_code =="") {
					...
					$("#btn-r-top").removeAttr("disabled");
					$("#validation-top").show();
					$('#validation-top').html('Kode harus diisi!');
					$("#validation-top").fadeTo(2000, 500).slideUp(500, function() {
	                	$("#validation-top").slideUp(500);
	                });	
				}else if (term =="") {
					$("#btn-s-top").removeAttr("disabled");
					$("#btn-rs-top").removeAttr("disabled");
					$("#btn-r-top").removeAttr("disabled");
					$("#validation-top").show();
					$('#validation-top').html('Term harus diisi!');
					$("#validation-top").fadeTo(2000, 500).slideUp(500, function() {
	                	$("#validation-top").slideUp(500);
	                });	
				}else{
					...
					$("#btn-r-top").removeAttr("disabled");
					$("#validation-top").show();
					$('#validation-top').html('Harap isi semua data yang dibutuhkan!');
					$("#validation-top").fadeTo(2000, 500).slideUp(500, function() {
	                	$("#validation-top").slideUp(500);
	                });
				}
		...
		$('#btn-s-top').on('click', function(e) {
			...
			if(top_code!="" && term!="" && (is_active !="" || is_non !="" )){
				var oTable = $('#datatable_top').DataTable();
				oTable.draw();
				...
				if (top_code =="") {
					...
					$("#btn-r-top").removeAttr("disabled");
					$("#validation-top").show();
					$('#validation-top').html('Kode Term harus diisi!');
					$("#validation-top").fadeTo(2000, 500).slideUp(500, function() {
	                	$("#validation-top").slideUp(500);
	                });	
				}else if (term =="") {
					...
					$("#btn-r-top").removeAttr("disabled");
					$("#validation-top").show();
					$('#validation-top').html('Nama Term harus diisi!');
					$("#validation-top").fadeTo(2000, 500).slideUp(500, function() {
	                	$("#validation-top").slideUp(500);
	                });	
				}else{
		...
		$('#btn-rs-top').on('click', function(e) {
			e.preventDefault();
			save_top();
			if(is_valid != 0){
				$("#validation-s-top").show();
				$('#validation-s-top').html('Data berhasil ditambahkan!');
				$("#validation-s-top").fadeTo(2000, 500).slideUp(500, function() {
                	$("#validation-s-top").slideUp(500);
                });
			}
			var $t = $(this),
		...
		$('#57F').on('click', function() {
			if(check_user_right(this.id) == 0){
				$('#modal_unauthorized').modal('show');
			}			
		});
		$('#57E').on('click', function() {
			if(check_user_right(this.id) == 0){
				$('#modal_unauthorized').modal('show');
			}			
		});
		$('#57D').on('click', function() {
			if(check_user_right(this.id) == 0){
				$('#modal_unauthorized').modal('show');
			}			
		});
		// action edit clicked row
		function cae_top(id_top) {
			var x = document.getElementById("57C");
	...
	            $('#id_top').val(id_top);
	            $.ajax({
					url: "<?php echo base_url("top/get_edit_data");?>",
					type: "POST",
					data: {
						id_top: id_top
					},
					...
					success: function(dataResult){
						...
						$('#id_top').val(json_data.top_id);
						$('#top_code').val(json_data.top_code);
						$('#term').val(json_data.term);
						$('#is_default_term').prop('checked',json_data.is_default_term);

						var status = json_data.status;

						// console.log(status);
						if(status == 0 ){
							$('#is_act_top').prop('checked', true);
							$('#is_non_top').prop('checked', false);
						}else{
							$('#is_act_top').prop('checked', false);
							$('#is_non_top').prop('checked', true);
						}
		...
		$('#57B').on('click', function() {
				
			if(check_user_right(this.id) == 0){
				$('#modal_unauthorized').modal('show');
			}else{
				$('#is_act_top').attr('checked', true);
				$('#is_act_top').closest('span').addClass('checked');
				$('#is_non_top').attr('checked', false);
				$('#is_non_top').closest('span').removeClass('checked');
		...
		function search_top() {
			...
			var urlist = "<?php echo base_url('top/page');?>";
		...
		$('#btn-rf-top').on('click', function(e) {
		...
		//action btn cari
		$('#btn-sf-top').on('click', function(e) {
			search_top();
		});
		...
		$(document).on('click', '#btn-pdf-grp', function(e){
			...
			var is_ar = $("#chk-arsip").prop("checked");
			var urlist = "<?php echo base_url('top/print_pdf');?>";
		...
		$(document).on('click', '#btn-csv-grp', function(e){
			...
			var is_ar = $("#chk-arsip").prop("checked");
			var urlist = "<?php echo base_url('top/print_csv');?>";
		...
		$(document).on('click', '#btn-exc-grp', function(e){
			...
			var is_ar = $("#chk-arsip").prop("checked");
			var urlist = "<?php echo base_url('top/print_exc');?>";
